...
@Client.on_message(filters.command("youtube@Mitsuri_Kanroji_Leech_Bot"))
async def iytdl_inline(client: Client, message: Message):
    ...
    if not input_url:
        x = await message.reply_text("Want /ban?👀")
        await asyncio.sleep(5)
        await x.delete()
        return
    x = await message.reply_text(f"Now Wait For A Bit👀👀<code>'{input_url}'</code>")
